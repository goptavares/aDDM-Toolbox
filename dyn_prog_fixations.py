...
def run_analysis(data, d, theta, std):
    ...
    subjects = data.rt.keys()
    likelihood = 0

    for subject in subjects:
        print 'Running subject ' + subject + '...'
        trials = data.rt[subject].keys()
        for trial in trials:
            if trial % 200 == 0:
            ...
            itemFixTime = 0
            transitionTime = 0
            for fItem, fTime in zip(data.fixItem[subject][trial],
                data.fixTime[subject][trial]):
            ...
            for fItem, fTime in zip(data.fixItem[subject][trial],
                ...
                if fItem == 1:  # subject is looking left.
                    valueLooking = data.leftValue[subject][trial]
                    valueNotLooking = data.rightValue[subject][trial]
                elif fItem == 2:  # subject is looking right.
                    valueLooking = data.rightValue[subject][trial]
                    valueNotLooking = data.leftValue[subject][trial]
                else:
                    continue
                ...
                for t in xrange(0, int(fTime/timeStep)):
                    ...
                    changeDown = ((barrierDown[time] * np.ones(states.size))
                        - states)
            ...
            # final choice.
            try:
                if data.choice[subject][trial] == -1:  # choice was left.
                    likelihood -= np.log(probUpCrossing[-1])
                elif data.choice[subject][trial] == 1:  # choice was right.
